...
const DataViewer = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Load report
      const reportPaths = ['/AVECapstone/ANALYSIS_REPORT.md', '/ANALYSIS_REPORT.md', './ANALYSIS_REPORT.md'];
      ...
      // Load CSV
      const csvPaths = ['/AVECapstone/scenario_analysis_data_100.csv', '/scenario_analysis_data_100.csv', './scenario_analysis_data_100.csv'];
      let csvLoaded = false;
      ...
      if (!csvLoaded) {
        setError('Could not load CSV data. The file may not be available.');
      }
    } catch (err) {
      setError('Error loading data: ' + err.message);
      console.error('DataViewer error:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const renderMarkdown = (text) => {
    if (!text) return <p className="text-gray-600">No report content available.</p>;
            ...
            <div className="prose max-w-none">
              <div className="bg-gray-50 rounded-lg p-6 border-2 border-gray-200">
                {reportContent ? (
                  ...
                  <p className="text-gray-600">Report content not available. The ANALYSIS_REPORT.md file may not be accessible.</p>
                )}
              </div>
            </div>
          )}
            ...
            <div>
              <div className="bg-gray-50 rounded-lg p-4 mb-4 border-2 border-gray-200">
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Connectedness Level</label>
                    <select
              ...
              {csvData.length === 0 ? (
                <div className="bg-yellow-50 border-2 border-yellow-200 rounded-lg p-6 text-center">
                  <p className="text-yellow-800">No CSV data loaded. The scenario_analysis_data_100.csv file may not be accessible.</p>
                </div>
              ) : (
